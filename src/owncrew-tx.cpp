...
static void SetupOwnCrewTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " owncrew-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The owncrew-tx tool is used for creating and modifying owncrew transactions.\n\n"
                "owncrew-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded owncrew transaction, or with \"-create [commands]\" to create a hex-encoded owncrew transaction.\n"
                "\n"
                "Usage: owncrew-tx [options] <hex-tx> [commands]\n"
                "or:    owncrew-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded owncrew transaction
