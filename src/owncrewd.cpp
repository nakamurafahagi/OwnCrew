...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/owncrew.conf are parsed in qt/owncrew.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see owncrewd -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (owncrewd) is a headless program that connects to the OwnCrew network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the OwnCrew network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for owncrewd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect owncrewd signal handlers
    noui_connect();
