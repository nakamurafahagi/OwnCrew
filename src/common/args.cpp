...
const char * const BITCOIN_CONF_FILENAME = "owncrew.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //owncrew-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\OwnCrew
    //   new: C:\Users\Username\AppData\Local\OwnCrew
    // macOS: ~/Library/Application Support/OwnCrew
    // Unix-like: ~/.owncrew
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "OwnCrew";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "OwnCrew";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/OwnCrew";
#else
    // Unix-like
    return pathRet / ".owncrew";
#endif
#endif
}
