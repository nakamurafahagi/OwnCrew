...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<OwnCrewUnit>("OwnCrewUnit");
#else
    qRegisterMetaType<OwnCrewUnit>("OwnCrewUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. owncrew_de.qm (shortcut "de" needs to be defined in owncrew.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. owncrew_de_DE.qm (shortcut "de_DE" needs to be defined in owncrew.qrc)
...
static const char* qt_argv = "owncrew-qt";

OwnCrewApplication::OwnCrewApplication()
...
void OwnCrewApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the OwnCrewApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", OwnCrewGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void OwnCrewApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool OwnCrewApplication::createOptionsModel(bool resetSettings)
{
...
void OwnCrewApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new OwnCrewGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &OwnCrewGUI::quitRequested, this, &OwnCrewApplication::requestShutdown);
...
void OwnCrewApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void OwnCrewApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool OwnCrewApplication::baseInitialize()
{
    return node().baseInitialize();
}

void OwnCrewApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &OwnCrewApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &OwnCrewApplication::handleRunawayException);
    connect(this, &OwnCrewApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &OwnCrewApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void OwnCrewApplication::parameterSetup()
{
...
void OwnCrewApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void OwnCrewApplication::requestInitialize()
{
...
void OwnCrewApplication::requestShutdown()
{
...
void OwnCrewApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // owncrew: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OwnCrewGUI::handlePaymentRequest);
            connect(window, &OwnCrewGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void OwnCrewApplication::handleRunawayException(const QString &message)
{
...
void OwnCrewApplication::handleNonFatalException(const QString& message)
{
...
WId OwnCrewApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool OwnCrewApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", OwnCrewGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(owncrew);
    Q_INIT_RESOURCE(owncrew_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see owncrew-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see owncrew-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse owncrew.conf, determine network, switch to network specific
    ...
    // owncrew: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
