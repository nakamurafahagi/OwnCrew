// Copyright (c) 2015-2022 The OwnCrew Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<OwnCrewAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = OwnCrewUnits::formatWithUnit(unit, expected_balance, false, OwnCrewUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_owncrew-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_owncrew-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_owncrew-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    OwnCrewAmountField* amountInput = receiveCoinsDialog.findChild<OwnCrewAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_owncrew-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
