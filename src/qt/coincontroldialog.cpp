...
{
    GUIUtil::setClipboard(OwnCrewUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    OwnCrewUnit nDisplayUnit = OwnCrewUnit::OWC;
    ...
    l2->setText(OwnCrewUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(OwnCrewUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(OwnCrewUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(OwnCrewUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    OwnCrewUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show owncrew address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, OwnCrewUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, OwnCrewUnits::format(nDisplayUnit, nSum));
