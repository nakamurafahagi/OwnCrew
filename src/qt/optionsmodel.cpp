...
/** Call node.updateRwSetting() with OwnCrew 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because OwnCrew 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/owncrew/owncrew/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because owncrew 22.x will not try to read these.
...
//! Convert enabled/size values to owncrew -prune setting.
...
//! Get pruning enabled value to show in GUI from owncrew -prune setting.
...
//! Get pruning size value to show in GUI from owncrew -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOwnCrewUnit")) {
        settings.setValue("DisplayOwnCrewUnit", QVariant::fromValue(OwnCrewUnit::OWC));
    }
    QVariant unit = settings.value("DisplayOwnCrewUnit");
    if (unit.canConvert<OwnCrewUnit>()) {
        m_display_owncrew_unit = unit.value<OwnCrewUnit>();
    } else {
        m_display_owncrew_unit = OwnCrewUnit::OWC;
        settings.setValue("DisplayOwnCrewUnit", QVariant::fromValue(m_display_owncrew_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if owncrew.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_owncrew_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OwnCrewUnit>() == m_display_owncrew_unit) return;
    m_display_owncrew_unit = new_unit.value<OwnCrewUnit>();
    QSettings settings;
    settings.setValue("DisplayOwnCrewUnit", QVariant::fromValue(m_display_owncrew_unit));
    Q_EMIT displayUnitChanged(m_display_owncrew_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/owncrew/owncrew/pull/8273
    ...
    // (https://github.com/owncrew-core/gui/issues/567).
