// Copyright (c) 2011-2022 The OwnCrew Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <owncrew-build-config.h> // IWYU pragma: keep

#include <qt/owncrewgui.h>

#include <qt/owncrewunits.h>
...
const std::string OwnCrewGUI::DEFAULT_UIPLATFORM =
...
OwnCrewGUI::OwnCrewGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &OwnCrewGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &OwnCrewGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &OwnCrewGUI::showModalOverlay);
...
void OwnCrewGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a OwnCrew address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and owncrew: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &OwnCrewGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &OwnCrewGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &OwnCrewGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your OwnCrew addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified OwnCrew addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed OwnCrew Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed OwnCrew Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a owncrew: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible OwnCrew command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &OwnCrewGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &OwnCrewGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &OwnCrewGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &OwnCrewGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &OwnCrewGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &OwnCrewGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &OwnCrewGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &OwnCrewGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &OwnCrewGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &OwnCrewGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &OwnCrewGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &OwnCrewGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &OwnCrewGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &OwnCrewGUI::showDebugWindow);
}

void OwnCrewGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void OwnCrewGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &OwnCrewGUI::setCurrentWalletBySelectorIndex);
...
void OwnCrewGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &OwnCrewGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &OwnCrewGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &OwnCrewGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &OwnCrewGUI::showProgress);
...
#ifdef ENABLE_WALLET
void OwnCrewGUI::enableHistoryAction(bool privacy)
{
...
void OwnCrewGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &OwnCrewGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &OwnCrewGUI::removeWallet);
...
WalletController* OwnCrewGUI::getWalletController()
{
    return m_wallet_controller;
}

void OwnCrewGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &OwnCrewGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &OwnCrewGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &OwnCrewGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &OwnCrewGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &OwnCrewGUI::incomingTransaction);
    connect(this, &OwnCrewGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void OwnCrewGUI::removeWallet(WalletModel* walletModel)
{
...
void OwnCrewGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void OwnCrewGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void OwnCrewGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OwnCrewGUI::setWalletActionsEnabled(bool enabled)
{
...
void OwnCrewGUI::createTrayIcon()
{
...
void OwnCrewGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &OwnCrewGUI::toggleHidden);
...
void OwnCrewGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void OwnCrewGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void OwnCrewGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void OwnCrewGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void OwnCrewGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void OwnCrewGUI::openClicked()
{
...
void OwnCrewGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void OwnCrewGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void OwnCrewGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void OwnCrewGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void OwnCrewGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void OwnCrewGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void OwnCrewGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void OwnCrewGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to OwnCrew network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void OwnCrewGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void OwnCrewGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void OwnCrewGUI::updateHeadersSyncProgressLabel()
{
...
void OwnCrewGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void OwnCrewGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &OwnCrewGUI::quitRequested);
...
void OwnCrewGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void OwnCrewGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &OwnCrewGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void OwnCrewGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void OwnCrewGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &OwnCrewGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &OwnCrewGUI::show);
                e->ignore();
            }
        }
    }
...
void OwnCrewGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void OwnCrewGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void OwnCrewGUI::incomingTransaction(const QString& date, OwnCrewUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(OwnCrewUnits::formatWithUnit(unit, amount, true));
...
void OwnCrewGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OwnCrewGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool OwnCrewGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool OwnCrewGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OwnCrewGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void OwnCrewGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void OwnCrewGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void OwnCrewGUI::updateProxyIcon()
{
    std::string ip_port;
...
void OwnCrewGUI::updateWindowTitle()
{
...
void OwnCrewGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void OwnCrewGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OwnCrewGUI::detectShutdown()
{
...
void OwnCrewGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void OwnCrewGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(OwnCrewGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = OwnCrewGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void OwnCrewGUI::subscribeToCoreSignals()
{
...
void OwnCrewGUI::unsubscribeFromCoreSignals()
{
...
bool OwnCrewGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<OwnCrewUnit> units = OwnCrewUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const OwnCrewUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, OwnCrewUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const OwnCrewUnit u : OwnCrewUnits::availableUnits()) {
        menu->addAction(OwnCrewUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(OwnCrewUnit newUnits)
{
    setText(OwnCrewUnits::longName(newUnits));
}
