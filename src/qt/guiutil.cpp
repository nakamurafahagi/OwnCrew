...
#include <qt/owncrewaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a OwnCrew address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new OwnCrewAddressEntryValidator(parent));
    widget->setCheckValidator(new OwnCrewAddressCheckValidator(parent));
}
...
bool parseOwnCrewURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no owncrew: URI
    if(!uri.isValid() || uri.scheme() != QString("owncrew"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!OwnCrewUnits::parse(OwnCrewUnit::OWC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseOwnCrewURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseOwnCrewURI(uriInstance, out);
}

QString formatOwnCrewURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("owncrew:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(OwnCrewUnits::format(OwnCrewUnit::OWC, info.amount, false, OwnCrewUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openOwnCrewConf()
{
    ...
    /* Open owncrew.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OwnCrew.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OwnCrew (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("OwnCrew (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for OwnCrew*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "owncrew.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("owncrew-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a owncrew.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=OwnCrew\n";
        else
            optionFile << strprintf("Name=OwnCrew (%s)\n", ChainTypeToString(chain));
