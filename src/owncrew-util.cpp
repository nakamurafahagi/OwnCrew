...
static void SetupOwnCrewUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " owncrew-util utility version " + FormatFullVersion() + "\n";

        if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The owncrew-util tool provides owncrew related functionality that does not rely on the ability to access a running node. Available [commands] are listed below.\n"
                "\n"
                "Usage:  owncrew-util [options] [command]\n"
                "or:     owncrew-util [options] grind <hex-block-header>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
