 ...
 */
static const char* BITCOIN_PID_FILENAME = "owncrewd.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    // UPnP support was dropped. We keep `-upnp` as a hidden arg to display a more user friendly error when set. TODO: remove (here and below) for 30.0. NOTE: removing this option may prevent the GUI from starting, see https://github.com/owncrew-core/gui/issues/843.
...
{
    // Make sure only a single OwnCrew process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if owncrew is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if owncrew is started while in a temporary directory.\n",
