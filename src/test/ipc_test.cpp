// Copyright (c) 2023 The OwnCrew Core developers
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_owncrew", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_owncrew", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_owncrew", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_owncrew", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_OwnCrew Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_owncrew.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("owncrew_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("owncrew_sock1_XXXXXX")),
    };
