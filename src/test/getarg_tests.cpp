...
void ResetArgs(ArgsManager& local_args, const std::string& strArg)
{
    ...
    vecArg.insert(vecArg.begin(), "testowncrew");
...
BOOST_AUTO_TEST_CASE(patharg)
{
    ...
    const fs::path absolute_path{"/home/user/.owncrew"};
    ResetArgs(local_args, "-dir=/home/user/.owncrew");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);

    ResetArgs(local_args, "-dir=/root/../home/user/.owncrew");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);
    ...
    const fs::path relative_path{"user/.owncrew"};
    ResetArgs(local_args, "-dir=user/.owncrew");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);

    ResetArgs(local_args, "-dir=somewhere/../user/.owncrew");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);
    ...
    // line arguments or owncrew.conf values. Currently the -dir= case cannot be
