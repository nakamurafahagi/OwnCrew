#!/usr/bin/env bash
# Copyright (c) 2019-2022 The OwnCrew Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/owncrew-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our OwnCrew Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built OwnCrew Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    case "$HOST" in
        *darwin*)
            ...
            mv build/dist/OwnCrew-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
    ...
    (
        ...
        # copy over the example owncrew.conf file. if contrib/devtools/gen-owncrew-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/owncrew.conf" "${DISTNAME}/"
