# Bootstrappable OwnCrew Core Builds

This directory contains the files necessary to perform bootstrappable OwnCrew
Core builds.
...
To build OwnCrew Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      OwnCrew Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/owncrew-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/owncrew-core/guix.sigs).
