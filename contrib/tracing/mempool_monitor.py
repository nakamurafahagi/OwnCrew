...
""" Example logging OwnCrew Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into owncrewd with pid {pid}")
    owncrewd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    owncrewd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    owncrewd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    owncrewd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    owncrewd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[owncrewd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of owncrewd>")
        exit(1)
