...
Tracing (USDT) support for OwnCrew Core on Linux using. For more information on
USDT support in OwnCrew Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `owncrewd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `owncrewd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[OwnCrew Core USDT documentation] on how to list available tracepoints in your
`owncrewd` binary.

[OwnCrew Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof owncrewd)
```
...
OwnCrew P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof owncrewd)
```
...
In a different terminal, starting OwnCrew Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/owncrewd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof owncrewd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof owncrewd)
```
