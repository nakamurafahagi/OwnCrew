#!/usr/bin/env python3
# Copyright (c) 2020-2022 The OwnCrew Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing owncrew-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='owncrew-cli', help='owncrew-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='OwnCrew address to which the faucet should send')
...
parser.add_argument('owncrew_cli_args', nargs='*', help='Arguments to pass on to owncrew-cli (default: -signet)')

args = parser.parse_args()

if args.owncrew_cli_args == []:
    args.owncrew_cli_args = ['-signet']


def owncrew_cli(rpc_command_and_params):
    argv = [args.cmd] + args.owncrew_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = owncrew_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = owncrew_cli(['getnewaddress', 'faucet', 'bech32'])
