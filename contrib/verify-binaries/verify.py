...
"""Script for verifying OwnCrew Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from owncrewcore.org and owncrew.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/owncrew-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://owncrewcore.org"
HOST2 = "https://owncrew.org"
VERSIONPREFIX = "owncrew-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the OwnCrew community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"owncrew_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by owncrewcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the owncrew release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes owncrew.org lags behind owncrewcore.org.)')
    )
