OwnCrew Core version 0.18.0 is now available from:

  <https://owncrewcore.org/bin/owncrew-core-0.18.0/>
...
`/Applications/OwnCrew-Qt` (on Mac) or `owncrewd`/`owncrew-qt` (on
Linux).
...
recommended to use OwnCrew Core on unsupported systems.
...
10.10. Additionally, OwnCrew Core does not yet change appearance when
...
`contrib/init/owncrewd.service`, it has been changed to use
`/var/lib/owncrewd` as the data directory instead of
`~owncrew/.owncrew`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/owncrewd` will
exist has enough space (check using `df -h /var/lib/owncrewd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/owncrew/owncrew/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://owncrewcore.org/en/contact/) the OwnCrew Core
...
The systemd init file (`contrib/init/owncrewd.service`) has been changed
to use `/var/lib/owncrewd` as the data directory instead of
`~owncrew/.owncrew`. This change makes OwnCrew Core more consistent with
...
`datadir` option in `/etc/owncrew/owncrew.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/owncrew/owncrew.conf`.


Documentation
-------------

- A new short [document](https://github.com/owncrew/owncrew/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/owncrew/owncrew/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/owncrew/owncrew/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/owncrew/owncrew/blob/master/doc/owncrew-conf.md)
  about the `owncrew.conf` file describes how to use it to configure
  OwnCrew Core.

- A new document introduces OwnCrew Core's BIP174 [Partially-Signed
  OwnCrew Transactions
  (PSBT)](https://github.com/owncrew/owncrew/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/owncrew/owncrew/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent OwnCrew-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  OwnCrew Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/owncrew/owncrew/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/owncrew/owncrew/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when OwnCrew Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of OwnCrew Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/owncrew/owncrew/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/owncrew/owncrew/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart owncrewd with the `-deprecatedrpc=generate`
...
- A new `owncrew-wallet` tool is now distributed alongside OwnCrew
...
This section describes planned changes to OwnCrew Core that may affect
other OwnCrew software and services.

- Since version 0.16.0, OwnCrew Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with OwnCrew Core 0.20 (expected about
  a year after 0.18), OwnCrew Core will default to native segwit
  ...
  to bech32 addresses, and if the OwnCrew Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in OwnCrew Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any OwnCrew Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, OwnCrew Core
...
- A sub-project of OwnCrew Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with OwnCrew Core.  See their [project
  page](https://github.com/owncrew-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to OwnCrew Core's own implementation, although entropy
  gathered by OwnCrew Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves OwnCrew Core a
...
- On macOS, OwnCrew Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `owncrew_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libowncrew_server.a` (jnewbery)
...
- #13926 [Tools] owncrew-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like owncrewd (MarcoFalke)
...
- #14521 qt, docs: Fix `owncrew-qt -version` output formatting (hebasto)
...
- #11625 Add OwnCrewApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libowncrew_server` (laanwj)
...
- #15154 Configure: owncrew-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/owncrew-wallet (Sjors)
...
- #11526 Visual Studio build configuration for OwnCrew Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libowncrew_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in owncrew-util-test.py (isghe)
...
- #12775 Integration of property based testing into OwnCrew Core (Christewart)
...
- #12246 Bugfix: Only run owncrew-tx tests when owncrew-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_owncrew` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_owncrew` (MarcoFalke)
...
- #14186 owncrew-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_owncrew.conf` message on startup if conf file exists (leishman)
...
- #12255 Update owncrew.service to conform to init.md (dongcarl)
...
- #14497 Add doc/owncrew-conf.md (hebasto)
...
- #15354 Add missing `owncrew-wallet` tool manpages (MarcoFalke)
...
- #15765 correct owncrewconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support owncrew folder on OSX (JimmyMow)
