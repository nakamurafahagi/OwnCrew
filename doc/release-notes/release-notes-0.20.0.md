...
OwnCrew Core version 0.20.0 is now available from:

  <https://owncrewcore.org/bin/owncrew-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from OwnCrew Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since OwnCrew Core version 0.18.0.
...
- Testing or debugging of implementations of the OwnCrew P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of OwnCrew Core. OwnCrew Core logs debug messages
...
- OpenSSL is no longer used by OwnCrew Core.  (#17265)

- BIP70 support has been fully removed from OwnCrew Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/owncrew/owncrew/issues/16599),
  [PR #16702](https://github.com/owncrew/owncrew/pull/16702), and the
  `owncrewd help` for more information.  This option is experimental and
...
- All custom settings configured when OwnCrew Core starts are now
...
- The "Start OwnCrew Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://owncrewcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple OwnCrew Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed OwnCrew Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/owncrew-core/HWI) for
  signing.  Future versions of OwnCrew Core should support a GUI option
...
- OwnCrew Core's automatically-generated source code documentation is
  now available at https://doxygen.owncrewcore.org.  (#17596)
...
- The `owncrew-cli` utility used with the `-getinfo` parameter now
...
- The `owncrew-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `owncrewstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_owncrew-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_owncrew` binary (hebasto)
...
- #18702 Fix ASLR for owncrew-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate owncrew schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest owncrew.conf (sdaftuar)
...
- #18022 Fix appveyor `test_owncrew` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (owncrew-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone owncrew-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about OwnCrew Core files (hebasto)
...
- #17119 Fix broken owncrew-cli examples (andrewtoth)
...
- #17503 Remove bitness from owncrew-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.owncrewcore.org (laanwj)
...
- #17648 Rename wallet-tool references to owncrew-wallet (hel-o)
...
- #18341 Replace remaining literal OWC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On owncrewd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from owncrew-cli -getinfo (malevolent)
