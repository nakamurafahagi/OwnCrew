# CJDNS support in OwnCrew Core

It is possible to run OwnCrew Core over CJDNS, an encrypted IPv6 network that
...
done outside of OwnCrew Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the OwnCrew network and individual nodes.
...
before it will work with your OwnCrew Core node. This is described in steps
...
## Run OwnCrew Core with CJDNS

Once you are connected to the CJDNS network, the following OwnCrew Core
...
When enabled, this option tells OwnCrew Core that it is running in an
...
IPv6 local network. This helps OwnCrew Core perform better address management:
...
CJDNS support was added to OwnCrew Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `owncrew-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/owncrew/owncrew/blob/master/doc/tor.md) for
details.

## CJDNS-related information in OwnCrew Core

There are several ways to see your CJDNS address in OwnCrew Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `owncrew-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `owncrew-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `owncrew-cli help getnodeaddresses` for details.
