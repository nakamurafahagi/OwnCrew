# PSBT Howto for OwnCrew Core

Since OwnCrew Core 0.17, an RPC interface exists for Partially Signed OwnCrew
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/owncrew/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for OwnCrew transactions that are not fully signed
...
#### Multisig with multiple OwnCrew Core instances
...
OwnCrew Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `owncrew-cli` is used.
...
Later, when *V* OWC has been received on *Amulti*, and Bob and Carol want to
