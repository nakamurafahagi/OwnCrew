...
This guide describes how to build owncrewd, command-line utilities, and GUI on FreeBSD.
...
Now that `git` and all the required dependencies are installed, let's clone the OwnCrew Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `owncrewd` or `owncrew-qt`.
...
```
to: /path/to/owncrew/depends/x86_64-unknown-freebsd[release-number]
```
