# User-space, Statically Defined Tracing (USDT) for OwnCrew Core

OwnCrew Core includes statically defined tracepoints to allow for more
...
**Note**: OwnCrew Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to OwnCrew Core
...
Multiple tools can list the available tracepoints in a `owncrewd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in OwnCrew Core, use `info probes` in `gdb`:

```
$ gdb ./build/src/owncrewd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/src/owncrewd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/src/owncrewd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/src/owncrewd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in OwnCrew Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/src/owncrewd | grep NT_STAPSDT -A 4 -B 2
