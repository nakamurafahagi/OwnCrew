...
Below are some notes on how to build OwnCrew Core for Windows.

The options known to work for building OwnCrew Core on Windows are:
...
Note that for WSL the OwnCrew Core source path MUST be somewhere in the default mount file system, for
example /usr/src/owncrew, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\owncrew`, for example:

    cmake --install build --prefix /mnt/c/workspace/owncrew
