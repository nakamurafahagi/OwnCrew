...
**When configuring OwnCrew Core, CMake by default will ignore the depends output.** In
...
created. To use it during configuring OwnCrew Core:
...
variables will be set when generating the OwnCrew Core buildsystem. In this case, `-DENABLE_WALLET=OFF`.
