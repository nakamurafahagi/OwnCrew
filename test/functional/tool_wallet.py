...
class ToolWalletTest(OwnCrewTestFramework):
    ...
    def owncrew_wallet_process(self, *args):
        ...
        return subprocess.Popen([self.options.owncrewwallet] + default_args + list(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.owncrew_wallet_process(*args)
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your OwnCrew, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `owncrew-wallet help` raises an error. Use `owncrew-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `owncrew-wallet -help` for valid methods.')
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/owncrew/owncrew/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of owncrew-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of owncrew-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
    ...
    def test_dump_unclean_lsns(self):
        ...
        # Lastly kill owncrewd so that the LSNs don't get reset
