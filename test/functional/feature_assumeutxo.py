#!/usr/bin/env python3
# Copyright (c) 2021-present The OwnCrew Core developers
...
to a hash that has been compiled into owncrewd.
...
class AssumeutxoTest(OwnCrewTestFramework):
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test owncrewd should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test owncrewd should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test owncrewd should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
    ...
    def run_test(self):
        ...
        # fixed. See also https://github.com/owncrew/owncrew/issues/26245
