...
class AbandonConflictTest(OwnCrewTestFramework):
    ...
    def run_test(self):
        ...
        inputs = []
        # spend 10owc outputs from txA and txB
        ...
        # Identify the 14.99998owc output
        ...
        # Verify that B and C's 10 OWC outputs are available for spending again because AB1 is now conflicted
        assert_equal(alice.gettransaction(txAB1)["confirmations"], -1)
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance + Decimal("20"))
        balance = newbalance

        # Invalidate the block with the double spend. B & C's 10 OWC outputs should no longer be available
