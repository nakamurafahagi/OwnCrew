#!/usr/bin/env python3
# Copyright (c) 2014-present The OwnCrew Core developers
...
TMPDIR_PREFIX = "owncrew_func_test_"
...
class OwnCrewTestMetaClass(type):
    """Metaclass for OwnCrewTestFramework.

    Ensures that any attempt to register a subclass of `OwnCrewTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'OwnCrewTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("OwnCrewTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("OwnCrewTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class OwnCrewTestFramework(metaclass=OwnCrewTestMetaClass):
    """Base class for a owncrew test script.

    Individual owncrew test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave owncrewds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop owncrewds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use owncrew-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "owncrewd": ("owncrewd", "BITCOIND"),
            "owncrew-cli": ("owncrewcli", "BITCOINCLI"),
            "owncrew-util": ("owncrewutil", "BITCOINUTIL"),
            "owncrew-wallet": ("owncrewwallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: owncrewds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("OwnCrewRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'owncrewd', self.options.owncrewd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'owncrew-cli', self.options.owncrewcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a owncrewd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple owncrewd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the owncrewd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as owncrewd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    owncrewd=self.options.owncrewd,
                    owncrew_cli=self.options.owncrewcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in owncrew.conf
    ...
    def skip_if_no_owncrewd_tracepoints(self):
        """Skip the running test if owncrewd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("owncrewd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_owncrewd_zmq(self):
        """Skip the running test if owncrewd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("owncrewd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if owncrew-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("owncrew-wallet has not been compiled")

    def skip_if_no_owncrew_util(self):
        """Skip the running test if owncrew-util has not been compiled."""
        if not self.is_owncrew_util_compiled():
            raise SkipTest("owncrew-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if owncrew-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("owncrew-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether owncrew-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether owncrew-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_owncrew_util_compiled(self):
        """Checks whether owncrew-util was compiled."""
