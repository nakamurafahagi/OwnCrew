...
class BackwardsCompatibilityTest(OwnCrewTestFramework):
    ...
    def test_v19_addmultisigaddress(self):
        ...
        # Copy the 0.19 wallet to the last OwnCrew Core version and open it:
        shutil.copytree(
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            for node in legacy_only_nodes:
                # RPC loadwallet failure causes owncrewd to exit in <= 0.17, in addition to the RPC
        ...
        if self.options.descriptors:
            ...
            node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of OwnCrew Core")
        ...
        for node in descriptors_nodes if self.options.descriptors else legacy_nodes:
            ...
            # Make sure this wallet opens with only the migration warning. See https://github.com/owncrew/owncrew/pull/19054
