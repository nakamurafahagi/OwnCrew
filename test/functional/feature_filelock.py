...
"""Check that it's not possible to start a second owncrewd instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(OwnCrewTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second owncrewd instance using the same datadir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second owncrewd using the same datadir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second owncrewd instance shutdown
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second owncrewd instance using the same wallet")
                if descriptors:
